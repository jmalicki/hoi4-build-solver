...
def load_nodes_from_csv(path: str) -> List["Node"]:
    ...
    for _, row in df.iterrows():
        ...
        if effective_slots < 0:
            raise ValueError(
                f"Effective slots negative after subtracting Docks/Refineries for node {row['nodeName']}"
            )
        node = Node(
...
@click.option(
    ...
    required=False,
    type=str,
    help="Google Sheet URL (will read the active tab via CSV export)",
)
@click.option(
    "--target-type",
    "target_type",
    required=True,
    type=click.Choice(["military", "civilian", "factories"], case_sensitive=False),
    help="Type of target: military (military factories only), civilian (civilian factories only), or factories (total factories)",
)
@click.option(
    "--target",
    "target_value",
    required=True,
    type=int,
    help="Target value for the selected target type",
)
@click.option(
    "--moves-out",
    "moves_out",
...
@click.option(
    ...
    required=True,
    type=click.Path(writable=True, dir_okay=False),
    help="Output CSV for final state",
)
@click.option(
    "--no-prune",
    "no_prune",
    is_flag=True,
    default=False,
    help="Disable pruning (default: pruning enabled)",
)
@click.option(
    "--verbose/--quiet",
    "verbose",
    default=True,
    show_default=True,
    help="Print periodic progress (controls progress callback)",
)
@click.option(
    "--print-every",
    "print_every",
...
def main(
    ...
    if target_type_lower == "military":
        if target_value < init_total_mil:
            raise ValueError(
                f"Target military ({target_value}) is less than current total military ({init_total_mil})"
            )
        max_possible = sum(n.num_slots for n in nodes)
        if target_value > max_possible:
            raise ValueError(
                f"Infeasible target: target={target_value} exceeds capacity={max_possible}"
            )
        needed = max(0, target_value - init_total_mil)
        click.echo(
            f"Current military: {init_total_mil} | Target military: {target_value} | Additional needed: {needed} | Max possible: {max_possible}"
        )
    elif target_type_lower == "civilian":
        if target_value < init_total_civ:
            raise ValueError(
                f"Target civilian ({target_value}) is less than current total civilian ({init_total_civ})"
            )
        max_possible = sum(n.num_slots for n in nodes)
        if target_value > max_possible:
            raise ValueError(
        ...
        click.echo(
            f"Current civilian: {init_total_civ} | Target civilian: {target_value} | Additional needed: {needed} | Max possible: {max_possible}"
        )
    elif target_type_lower == "factories":
        if target_value < init_total_factories:
            raise ValueError(
                f"Target factories ({target_value}) is less than current total factories ({init_total_factories})"
            )
        max_possible = sum(n.num_slots for n in nodes)
        if target_value > max_possible:
            raise ValueError(
        ...
        click.echo(
            f"Current factories: {init_total_factories} | Target factories: {target_value} | Additional needed: {needed} | Max possible: {max_possible}"
        )
    else:
    ...
    rust_nodes = [
        (n.name, int(n.num_slots), int(n.num_infra), int(n.num_civilian), int(n.num_military))
        for n in nodes
    ]
    ...
    def _progress_cb(snap) -> bool:
        ...
        print(
            f"[A*] iters={iters} cost={cost:.4f} heap={heap} states={states} avg_f={avg_f:.4f} pruned={pruned} ub: best_ub={ub:.4f}",
            flush=True,
        )
    ...
    if verbose:
        print(
            f"[A*] invoking rust core: target_type={target_type_lower}, target={target_value}, nodes={len(rust_nodes)}, heuristic={heuristic_name}",
            flush=True,
        )
    try:
        ...
        moves, final_state_vec, total_cost = hoi4_mdp_core.solve_and_reconstruct(
            ...
            verbose=verbose,
            print_every=print_every,
            re_prune=not no_prune,
        )
    goal_state = tuple((int(i), int(c), int(m)) for (i, c, m) in final_state_vec)

    # Write moves CSV
    moves_df = pd.DataFrame(
        [{"step": i + 1, "nodeName": n, "action": a} for i, (n, a) in enumerate(moves)]
    )
