...
- **States**: A state encodes all nodes’ (`numInfra`, `numCivilian`,
  `numMilitary`). `numSlots` is per-node fixed input and is not part of
  the dynamic state. `totalCost` is accumulated via per-action costs and
  is not a state variable. Terminal states satisfy $\sum \text{numMilitary}
  = \text{targetMilitary}$.
