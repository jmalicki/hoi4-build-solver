...
**Potential issue**: Line 110 updates `best_ub = cost_value + ub_ns` when we don't prune. But `cost_value + ub_ns` is an
upper bound on paths _through_ that successor, not necessarily on the optimal solution. If we update `best_ub` to be too
low, we may prune optimal paths later.

**Key question**: Is `best_ub = min(best_ub, cost_value + ub_ns)` always safe? This assumes that the upper bound on a
successor is always ≥ the optimal cost through that successor, which requires the upper bound heuristic to be
admissible.
